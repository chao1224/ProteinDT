...
class SDE(abc.ABC):
    ...
    def reverse(self, score_fn, probability_flow=False):
        ...
        class RSDE(self.__class__):
            ...
            # def sde(self, feature, x, x_mask, condition, t):
            #     """Create the drift and diffusion functions for the reverse SDE/ODE."""
            #     drift, diffusion = sde_fn(x, t)
            #     score = score_fn(x, x_mask, condition, t)
            #     drift = drift - diffusion[:, None, None] ** 2 * score * (0.5 if self.probability_flow else 1.)
            #     # -------- Set the diffusion function to zero for ODEs. --------
            #     diffusion = 0. if self.probability_flow else diffusion
            #     return drift, diffusion

            def discretize(self, x, x_mask, condition, t):
                """Create discretized iteration rules for the reverse diffusion sampler."""
                f, G = discretize_fn(x, t)  # (B, max_seq_len, num_class), (B)
                score = score_fn(x, x_mask, condition, t)  # (B, max_seq_len, num_class)
                rev_f = f - G[:, None, None] ** 2 * score * (0.5 if self.probability_flow else 1.)  # (B, max_seq_len, num_class)
                rev_G = torch.zeros_like(G) if self.probability_flow else G  # (B)
                return rev_f, rev_G
...
class VPSDE(SDE):
    def __init__(self, beta_min=0.1, beta_max=20, N=1000):
        ...
        self.N = N
        self.discrete_betas = torch.linspace(beta_min / N, beta_max / N, N)  # \beta_1, \beta_2, ..., \beta_T
        self.alphas = 1. - self.discrete_betas  # \alpha_1, \alpha_2, ..., \alpha_T
        self.alphas_cumprod = torch.cumprod(self.alphas, dim=0)  # \bar\alpha_1, \bar\alpha_2, ..., \bar\alpha_T
...
class VESDE(SDE):
    ...
    def discretize(self, x, t):
        ...
        adjacent_sigma = torch.where(timestep == 0, torch.zeros_like(t), self.discrete_sigmas.to(t.device)[timestep - 1])
