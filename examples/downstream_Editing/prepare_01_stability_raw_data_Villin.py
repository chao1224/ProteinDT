...
if __name__ == "__main__":
    ...
    for idx, record in enumerate(data):
        if 'villin' not in record['topology']:
            continue
        protein_sequence = record["primary"]
        assert protein_sequence[:5] == "GSSGS"
        assert protein_sequence[-3:] == "GSS"
        protein_sequence = protein_sequence[5:-3]
        assert len(protein_sequence) == 35
    ...
    output_dir = "datasets_and_checkpoints/stability/Villin"
